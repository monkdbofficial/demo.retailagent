...
def multi_agent_workflow(csv_file_path: str):
    print("\n🔍 Upload Phase:")
    uploader_RES = uploader.invoke(
        f"Use the upload_and_visualize tool to upload this CSV file: {csv_file_path}")
    print(f"Uploader Result: {uploader_RES['output']}")

    print("\n🧠 Insight Phase:")
    insighter_RES = insighter.invoke(
        f"Use the generate_insights tool to extract insights from: {csv_file_path}")
    print(f"Insights Result: {insighter_RES['output']}")

    print("\n🛠️ Deploy Phase:")
    deployer_RES = deployer.invoke(
        f"Use deploy_dashboard to deploy results from: {csv_file_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run multi-agent orchestration pipeline on a CSV file")
