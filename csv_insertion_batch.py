...
def main(csv_file_path: str):
    ...
    logger.info(
        f"✅ Dask cluster up: {N_WORKERS} workers x {THREADS_PER_W} threads")
    ...
    logger.info(
        f"📂 Loaded CSV: {csv_file_path} with {len(ddf.columns)} columns")
    ...
    for col in ["product_id", "style_id", "title", "brand", "price", "mrp",
        ...
        if col not in ddf.columns:
            ddf[col] = None
            logger.warning(
                f"⚠️ Column {col} missing in input. Filling with None.")
    ...
    logger.info(
        f"✅ Inserted {total_inserted} records into {DB_SCHEMA}.{TABLE_NAME}")
